...
class EmailTriggerExternalModule extends AbstractExternalModule
{
    ...
    function getPreviewByRecord($project_id, $record, $index){
        #get data from the DB
        $form_name = empty($this->getProjectSetting('form-name'))?array():$this->getProjectSetting('form-name')[$index];
        $form_name_event =  empty($this->getProjectSetting('form-name-event'))?array():$this->getProjectSetting('form-name-event')[$index];
        $email_from = empty($this->getProjectSetting('email-from'))?array():$this->getProjectSetting('email-from')[$index];
        $email_subject =  empty($this->getProjectSetting('email-subject'))?array():$this->getProjectSetting('email-subject')[$index];
        $email_text =  empty($this->getProjectSetting('email-text'))?array():$this->getProjectSetting('email-text')[$index];
        $datapipe_var = $this->getProjectSetting("datapipe_var", $project_id);
        ...
        //Email Addresses
        $mail = $this->setEmailAddresses($mail, $project_id, $record, $form_name_event, $form_name, 1, $data, $index,\REDCap::isLongitudinal());
        ...
        $email_text = $this->setDataPipping($datapipe_var, $email_text, $project_id, $data, $record, $form_name_event, $form_name, 1,\REDCap::isLongitudinal());
        $email_subject = $this->setDataPipping($datapipe_var, $email_subject, $project_id, $data, $record, $form_name_event, $form_name, 1,\REDCap::isLongitudinal());
